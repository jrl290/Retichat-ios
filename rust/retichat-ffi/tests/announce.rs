...
fn test_announce_callback_fires_for_python_peer() {
    ...
    let _client_guard =
        helpers::ClientGuard::new(helpers::start_test_client(dir.path(), "AnnounceTest"));
    ...
    let rc =
        retichat_ffi::lxmf_client_watch(client, py_dest_hash.as_ptr(), py_dest_hash.len() as u32);
    assert_eq!(
        rc,
        0,
        "lxmf_client_watch(py) failed: {:?}",
        helpers::last_error_str()
    );
    helpers::done(s);
    ...
    let got = helpers::wait_until(&capture, Duration::from_secs(30), |events| {
        events.iter().any(|e| e.dest_hash == py_dest_hash)
    });
    {
        let (lock, _) = &*capture;
    ...
    unsafe {
        helpers::release_ctx_ptr::<helpers::AnnounceEvent>(ctx_ptr);
    }
    helpers::done(s);
